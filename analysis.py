...
def welchs_test(response_times_configs):
    ...
    for response_times_configs_combs in itertools.combinations(response_times_configs, 2):
        response_times_configs_1 = response_times_configs_combs[0]
        response_times_configs_2 = response_times_configs_combs[1]
        p_value = st.ttest_ind(response_times_configs_1["response_time"], response_times_configs_2["response_time"], equal_var=False).pvalue
        exists_significance_difference = True if p_value < p else False
        print(
            f"Experiment Type {response_times_configs_1['experiment_type']} and {response_times_configs_2['experiment_type']} -> p-value: {p_value}, Significance Difference: {exists_significance_difference}"
        )
...
if __name__ == "__main__":
    ...
    for data_path in data_paths:
        ...
        # ヒストグラムを描画
        draw_histograms(experiment_type, response_time)
